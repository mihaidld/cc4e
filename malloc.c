...
// added suffix _new to distinguish from names in stdlib.h
// and replaced deprecated sbrk by real malloc
// https://stackoverflow.com/questions/13159564/explain-this-implementation-of-malloc-from-the-kr-book

// #define NALLOC 1024 /* minimum #units to request */
#define NALLOC 3 /* minimum #units (block sizes) to request to OS*/

typedef long Align; /* for alignment to long boundary (instance of the most restrictive alignment type)*/
...
union header
{
    struct
    {
        /* next block if on free list 8 bytes*/
        union header *next;

        /* size of this block, big enough to contain the size of the biggest
         * object the host system can handle, on 64 bit system
         * it's typedef for unsigned long long so 8 bytes*/
        size_t size;
    } s;
...
/* Used to get an initial member for free list, when empty since static it's initialized at 0*/
static Header base;
static Header *freep = NULL; /* Starting point in linked list of free blocks, pointing to a block*/
// store total size (nunits) requested from OS
size_t allocated_size = 0;

void free_new(void *ptr);
void *malloc_new(size_t nbytes);
static Header *morecore_new(size_t nunits);
void check_free(void);

int main(void)
{
    int *pi = malloc_new(sizeof(int));
    *pi = 5;
    char *pc = malloc_new(10);
    strcpy(pc, "micky");
    float *pf = malloc_new(sizeof(float));
    *pf = 3.14;
...
/* malloc_new: general-purpose storage allocator */
void *malloc_new(size_t nbytes)
{
    Header *currp;
    Header *prevp;
     ...
     *
     * The extra + 1 is to include the unit of memory
    ...
    for (;; prevp = currp, currp = currp->s.next)
    {
        ...
        if (currp == freep)
        {
             ...
             */
            if ((currp = morecore_new(nunits)) == NULL)
            {
                return NULL;
            }
...
static Header *morecore_new(size_t nunits)
{
    ...
    Header *insertp; // Header ptr for pointer arithmetic and constructing header
    ...
    // need to remove flag -Wextra to compile since warning 'sbrk' is deprecated
    freemem = sbrk(nunits * sizeof(Header));
    // freemem = malloc(nunits * sizeof(Header));
...
/* free: put block pointed at by ptr in free list (called either by morecore
 * after getting new block from OS or by the program after malloc'ing)
 */
void free_new(void *ptr)
{
     ...
     * Cases possible:
       * 1 BASE -> BLOCK1 (currp) -> INSERTP -> BLOCK2 -> BASE between 2 blocks (if put there by program)
       * 2 BASE -> BLOCK1 -> BLOCK2 (currp)-> INSERTP -> BASE becomes rightmost (if put there by morecore)
       * 3 INSERTP -> BASE -> BLOCK1 -> BLOCK2 (currp) -> INSERTP becomes leftmost (but not possible normally)

     * In the typical circumstances (case 1) this would be the block
     ...
     * We traverse the list cheking next block if one of these 2 conditions are not met
     * However this check doesn't check for one such case (2), and misses another (3).
     * We still have to check for the cases where either the insertion block is
     * either to the left of every other block owned by malloc (case 3 that is
     * missed since the base block inside free list has lowest address since it's
     * global variable, lower than all the other blocks retrieved from sbrk
     * from uninitialized data segment), or to the right of every block owned by malloc (the case 2 not
    ...
    for (currp = freep; !((currp < insertp) && (insertp < currp->s.next)); currp = currp->s.next)
    {
        ...
        /* currp >= currp->s.next implies that the current block is the rightmost
         * block in the free list : cases 2 and 3.  Then if the insertion block is to the right of
         * that block, then it is the new rightmost block case 2; conversely if it is to
         * the left of the block that currp points to (which is the current leftmost
         * block), then the insertion block is the new leftmost block case 3.  Note that
     ...
     * right memory block received from OS.
     */
    freep = currp;
}

// Check malloc'ed space was freed (like valgrind or leaks)
void check_free(void)
{
    // store sum of free list block sizes
    size_t free_size = 0;

    // traverse free list if list has more than 1 block (base)
    if (freep != NULL && freep != freep->s.next)
    {
        Header *currp = freep;
        do
        {
            free_size += currp->s.size;
            currp = currp->s.next;
        } while (currp != freep);
    }

    if (free_size == allocated_size)
        printf("Allocated space was freed\n");
    else
        printf("Allocated space %li different from freed space %li\n", allocated_size, free_size);
}