...
int main(void)
{
    ...
    // store pointer to key returned by binary
    key *p;
    ...
    while ((t = getword(word, MAXWORD)) != EOF)
        if (t == LETTER)
            /*binary search for word in keytable and get pointer to element
            if found (not NULL) increment keyCount for this keyword */
            if ((p = binary(word, keytable, NKEYS)) != NULL)
                p->keycount++;

    // print count results for each keyword present
    // reset p to point to beginning of array keytable then use pointer arithmetic
